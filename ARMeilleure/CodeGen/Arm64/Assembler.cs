...
{
    // TODO: check all instructions against possible needed variants
    class Assembler
    {
        ...
        public void Add(Operand rd, Operand rn, Operand rm, ArmShiftType shiftType = ArmShiftType.Lsl, int shiftAmount = 0)
        {
            WriteInstructionAuto(0x11000000u, 0x0b000000u, rd, rn, rm, shiftType, shiftAmount);
        }
        ...
        {
            Trace.Assert(rd.Type.IsInteger() != rn.Type.IsInteger());
            Trace.Assert(rd.Type == OperandType.I64 || rn.Type == OperandType.I64 || !topHalf);
        ...
        public void Ins(Operand rd, Operand rn, int srcIndex, int dstIndex, int size)
        {
            ...
            Trace.Assert((uint)srcIndex < (16u >> size));
        ...
        public void Ldr(Operand rt, Operand rn)
        {
            if (rn.Kind == OperandKind.Memory)
            {
                ...
                {
                    Trace.Assert(memOp.Displacement == 0);
                    Trace.Assert(memOp.Scale == Multiplier.x1 || (int)memOp.Scale == GetScaleForType(rt.Type));
        ...
        public void MovSp(Operand rd, Operand rn)
        {
            ...
            {
                Add(rd, rn, Factory.Const(rd.Type, 0));
            }
            else
            {
                Mov(rd, rn);
        ...
        {
            Trace.Assert((hw & (rd.Type == OperandType.I64 ? 3 : 1)) == hw);
        ...
        {
            // FIXME: The instruction written here is for the 32 bits variant of RORV, is that intended???
            ...
            int shiftAmount = 0)
        {
            ...
            {
                Trace.Assert(shiftAmount == 0);
                int imm = rm.AsInt32();
                Trace.Assert((uint)imm == rm.Value);
        ...
        private void WriteInstructionAuto(
            ...
            int shiftAmount = 0)
        {
            Trace.Assert((uint)shiftAmount <= 4);
        ...
        {
            Trace.Assert((uint)size < 4);
            Trace.Assert((uint)index < (16u >> size), $"Invalid index {index} and size {size} combination.");
        ...
        private static uint EncodeSImm7(int value, int scale)
        {
            ...
            Trace.Assert(((int)imm << 25) >> (25 - scale) == value, $"Failed to encode constant 0x{value:X} with scale {scale}.");
            return imm;
        }
        ...
        private static uint EncodeSImm9(int value)
        {
            ...
            Trace.Assert(((int)imm << 23) >> 23 == value, $"Failed to encode constant 0x{value:X}.");
            return imm;
        }
        ...
        private static uint EncodeSImm19_2(int value)
        {
            ...
            Trace.Assert(((int)imm << 13) >> 11 == value, $"Failed to encode constant 0x{value:X}.");
            return imm;
        }
        ...
        private static uint EncodeSImm26_2(int value)
        {
            ...
            Trace.Assert(((int)imm << 6) >> 4 == value, $"Failed to encode constant 0x{value:X}.");
            return imm;
        }
        ...
        private static uint EncodeUImm4(int value)
        {
            ...
            Trace.Assert((int)imm == value, $"Failed to encode constant 0x{value:X}.");
            return imm;
        }
        ...
        private static uint EncodeUImm12(int value, int scale)
        {
            ...
            Trace.Assert((int)imm << scale == value, $"Failed to encode constant 0x{value:X} with scale {scale}.");
            return imm;
        }
        ...
        private static uint EncodeReg(Operand reg)
        {
            ...
            Trace.Assert(reg.Kind == OperandKind.Register);
            Trace.Assert(regIndex < 32);
            return regIndex;
        }
        ...
        public (byte[], RelocInfo) GetCode()
        {
            ...
            RelocInfo relocInfo = new RelocInfo(new RelocEntry[0]);
