...
namespace ARMeilleure.CodeGen.Arm64
{
    class PreAllocator
    {
                ...
                for (Operation node = block.Operations.First; node != default; node = nextNode)
                {
                    ...
                    switch (node.Instruction)
                    {
                        case Instruction.Call:
                            ...
                            HandleCall(block.Operations, node);
                            break;
                        ...
                        case Instruction.LoadArgument:
                            nextNode = HandleLoadArgument(cctx, block.Operations, preservedArgs, node);
                            break;
                        case Instruction.Return:
                            HandleReturn(block.Operations, node);
                            break;
                        case Instruction.Tailcall:
                            HandleTailcall(block.Operations, stackAlloc, node, node);
                            break;
                    }
                }
            }
        ...
        private static void HandleConstantRegCopy(IntrusiveList<Operation> nodes, Operation node)
        {
            ...
            for (int srcIndex = 2; srcIndex < node.SourcesCount; srcIndex++)
            {
                ...
                if (src.Kind == OperandKind.Constant)
                {
                    ...
                    {
                        src = AddXmmCopy(nodes, node, src);

                        node.SetSource(srcIndex, src);
                    }
                    else
                    {
                        src = AddCopy(nodes, node, src);
        ...
        private static void HandleCall(IntrusiveList<Operation> nodes, Operation node)
        {
        ...
        private static void HandleTailcall(IntrusiveList<Operation> nodes, StackAllocator stackAlloc, Operation node, Operation operation)
        {
            List<Operand> sources = new List<Operand>
            {
                operation.GetSource(0)
